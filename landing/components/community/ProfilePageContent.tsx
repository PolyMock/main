...
export default function ProfilePageContent({ address }: { address: string }) {
  ...
  const handleDeletePost = async (trade: TradeItem) => {
    if (!confirm(`Delete this ${trade.type === "trade" ? "trade" : "strategy"}? This cannot be undone.`)) return;
    const table = trade.type === "trade" ? "trades" : "backtest_strategies";
    const { error } = await supabase.from(table).delete().eq("id", trade.id).eq("user_id", user!.id);
    if (error) { alert("Failed to delete post."); return; }
    setTrades((prev) => prev.filter((t) => t.id !== trade.id || t.type !== trade.type));
  };
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-right shrink-0 flex items-center gap-3">
                      ...
                      {isOwner && (
                        <button
                          onClick={() => handleDeletePost(trade)}
